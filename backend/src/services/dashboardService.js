...
const getDashboard = async () => {
  const [woResult, itemResult, classifiedResult, reviewResult, hoursResult] = await Promise.all([
    ...
    pool.query(`SELECT COALESCE(SUM(COALESCE(ie.verification_mh, 0) + (COALESCE(ie.total_hours, 0) - COALESCE(ie.verification_mh, 0)) * COALESCE(woi.quantity, 1)), 0) AS total FROM item_estimations ie JOIN work_order_items woi ON woi.id = ie.work_order_item_id`),
  ]);
